"""
Code example number 3.
Use this template if:
i) you are using another model for calculating the pdf, cdf and the expected number of events (μ).
ii) there is a nonlinear dependence between the expected number of events (μ) and the cross-sections (σ).
"""
...
mus = [-np.sqrt(x)*np.log(x)*20*5.5 for x in sigmas]  # wichtig dass die hinzuzufügenden mus bis zumindest omega.set_mu_interval max gehen!
mus_1 = [-x*x*np.sin(x)*np.log(x)*1e8/9 for x in sigmas]
...
omega.get_limit_from_other_nonlinear_model()  # Call the function to determine limit values.
